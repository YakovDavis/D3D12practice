...
void Ssao::SetPSOs(ID3D12PipelineState* ssaoPso, ID3D12PipelineState* ssaoHorzBlurPso, ID3D12PipelineState* ssaoVertBlurPso)
{
    mSsaoPso = ssaoPso;
    mHorzBlurPso = ssaoHorzBlurPso;
    mVertBlurPso = ssaoVertBlurPso;
}
    ...
    int blurCount)
{
    ...
    cmdList->SetComputeRootDescriptorTable(2, mhRandomVectorMapGpuSrv);
    cmdList->SetComputeRootDescriptorTable(3, mhAmbientMap0GpuUav);

    cmdList->SetPipelineState(mSsaoPso);

    cmdList->Dispatch(SsaoMapWidth() / 16 + 1, SsaoMapHeight() / 16 + 1, 1);
...
{
    cmdList->SetPipelineState(mHorzBlurPso);
...
void Ssao::BlurAmbientMap(ID3D12GraphicsCommandList* cmdList, bool horzBlur)
{
    ...
    {
        cmdList->SetPipelineState(mVertBlurPso);
    ...
    cmdList->SetComputeRootDescriptorTable(2, inputSrv);

    cmdList->SetComputeRootDescriptorTable(3, outputUav);

    if (horzBlur == true)
    {
        cmdList->SetPipelineState(mHorzBlurPso);
        UINT numGroupsX = (UINT)ceilf(SsaoMapWidth() / 256.0f);
        cmdList->Dispatch(numGroupsX, SsaoMapHeight(), 1);
    }
    else
    {
        cmdList->SetPipelineState(mVertBlurPso);
        UINT numGroupsY = (UINT)ceilf(SsaoMapHeight() / 256.0f);
        cmdList->Dispatch(SsaoMapWidth(), numGroupsY, 1);
    }
