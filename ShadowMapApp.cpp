...
void ShadowMapApp::Draw(const GameTimer& gt)
{
    ...
    mCommandList->SetComputeRootSignature(mSsaoRootSignature.Get());
    mSsao->ComputeSsao(mCommandList.Get(), mCurrFrameResource, nullptr, 3);
...
void ShadowMapApp::UpdateSsaoCB(const GameTimer& gt)
{
...
void ShadowMapApp::BuildSsaoRootSignature()
{
    CD3DX12_DESCRIPTOR_RANGE texTable0;
...
void ShadowMapApp::BuildShadersAndInputLayout()
{
    ...
    mShaders["ssaoCS"] = d3dUtil::CompileShader(L"Shaders\\Ssao.hlsl", nullptr, "CS", "cs_5_0");
    mShaders["ssaoHorzBlurCS"] = d3dUtil::CompileShader(L"Shaders\\SsaoBlur.hlsl", nullptr, "HorzBlurCS", "cs_5_0"); //new
    mShaders["ssaoVertBlurCS"] = d3dUtil::CompileShader(L"Shaders\\SsaoBlur.hlsl", nullptr, "VertBlurCS", "cs_5_0"); //new
...
void ShadowMapApp::BuildPSOs()
{
    ...
    //
    // PSO for horizontal SSAO blur.
    //
    D3D12_COMPUTE_PIPELINE_STATE_DESC ssaoHorzBlurPsoDesc = ssaoPsoDesc;
    ssaoHorzBlurPsoDesc.pRootSignature = mSsaoRootSignature.Get();
    ssaoHorzBlurPsoDesc.CS =
    {
        reinterpret_cast<BYTE*>(mShaders["ssaoHorzBlurCS"]->GetBufferPointer()),
        mShaders["ssaoHorzBlurCS"]->GetBufferSize()
    };
    ssaoHorzBlurPsoDesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
    ThrowIfFailed(md3dDevice->CreateComputePipelineState(
        &ssaoHorzBlurPsoDesc, IID_PPV_ARGS(&mPSOs["ssaoHorzBlur"])));

    //
    // PSO for vertical SSAO blur.
    //
    D3D12_COMPUTE_PIPELINE_STATE_DESC ssaoVertBlurPsoDesc = ssaoPsoDesc;
    ssaoVertBlurPsoDesc.pRootSignature = mSsaoRootSignature.Get();
    ssaoVertBlurPsoDesc.CS =
    {
        reinterpret_cast<BYTE*>(mShaders["ssaoVertBlurCS"]->GetBufferPointer()),
        mShaders["ssaoVertBlurCS"]->GetBufferSize()
    };
    ssaoVertBlurPsoDesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
    ThrowIfFailed(md3dDevice->CreateComputePipelineState(
        &ssaoVertBlurPsoDesc, IID_PPV_ARGS(&mPSOs["ssaoVertBlur"])));
...
void ShadowMapApp::DrawNormalsAndDepth()
{
    mCommandList->RSSetViewports(1, &mScreenViewport);
    mCommandList->RSSetScissorRects(1, &mScissorRect);
...
CD3DX12_CPU_DESCRIPTOR_HANDLE ShadowMapApp::GetDsv(int index)const
{
...
CD3DX12_CPU_DESCRIPTOR_HANDLE ShadowMapApp::GetRtv(int index)const
{
