...
struct LightInfo
{
    float4 Position; // Light position in world coords
    float3 L;        // Intensity
}
...
sampler1DArray DictionaryTex; // Array of 1D textures, containing the marginal distributions (the dictionary)
...
float3 f_diffuse(float3 wo, float3 wi)
{
    ...
    return float3(0.8, 0., 0.) * m_i_pi * wi.z;
}
...
float P22_theta_alpha(float2 slope_h, int l, int s0, int t0)
{
    ...
    float2 scaleFactor = float2(Material.Alpha_x / Dictionary.Alpha,
    ...
    float3 P_i = textureLod(DictionaryTex, float2(texCoordX, l_dist * Dictionary.N / 3 + distIdxXOver3), 0).rgb;
    float3 P_j = textureLod(DictionaryTex, float2(texCoordY, l_dist * Dictionary.N / 3 + distIdxYOver3), 0).rgb;
...
float P22__P_(int l, float2 slope_h, float2 st, float2 dst0, float2 dst1)
{
...
//==================================================== Alg. 1, Eq. 14 =====================================================
//=========================================================================================================================
float3 f_P(float3 wo, float3 wi)
{
...
{
    float3 binormal = cross(VertexNorm, VertexTang);
    ...
    float3 wi = toLocal * normalize(Light.Position.xyz - VertexPos);
    wi = normalize(wi);
    float3 wo = toLocal * normalize(CameraPosition - VertexPos);
    wo = normalize(wo);

    float3 radiance_specular = float3(0);
    float3 radiance_diffuse = float3(0);
    float3 radiance = float3(0);

    float distanceSquared = distance(VertexPos, Light.Position.xyz);
    distanceSquared *= distanceSquared;
    float3 Li = Light.L / distanceSquared;
